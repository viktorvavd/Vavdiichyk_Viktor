...
public class Decriptor extends Thread{
    ...
    public void decript(byte [] message) throws Exception {
        ...
        byte[] header = ByteBuffer.allocate(14)
                ...
                .array();
        if(buffer.getShort(14) != crc16(header)){
            throw new Exception("Wrong CRC_16");
        }

        byte[] encryptMess = new byte[message.length-26];
        System.arraycopy(message,24,encryptMess,0,encryptMess.length);
       ...
       this.command = new Message(buffer.getInt(16),buffer.getInt(20),decryptMessage);

        if(buffer.getShort(message.length-2) != crc16(decryptMessage)){
            throw new Exception("Wrong CRC_16_Final");
        }
        //System.out.println(Arrays.toString(decryptMessage));
    }
